...
from easyprotocol.fields.date_time import (
...
def check_datetime_properties(
    ...
    assert (
        obj.date_string_format == tst.date_string_format
    ), f"{obj}: obj.format is not the expected value ({obj.date_string_format} != expected value: {tst.date_string_format})"
    assert (
...
def check_datetime_strings(
    obj: DateTimeField,
    tst: ParseData,
) -> None:
    assert (
        tst.string_format.format(cast("datetime", tst.value).strftime(tst.date_string_format)) == obj.value_as_string
    ), (
...
def check_datetime(
    ...
    check_datetime_value(
        obj=obj,
        tst=tst,
    )
    check_datetime_properties(
        obj=obj,
        tst=tst,
    )
    check_datetime_children(
        obj=obj,
        tst=tst,
    )
    check_datetime_strings(
        obj=obj,
        tst=tst,
    )
...
class TestDateTimeField:
    def test_DateTimeField_create_empty_big_endian(self) -> None:
        value = 0
        ...
        tst = ParseData(
            ...
            string_format=DATE_FIELD_STRING_FORMAT,
            byte_data=byte_data,
            bits_data=bits_data,
            parent=None,
            endian="big",
    ...
    def test_DateTimeField_create_empty_little_endian(self) -> None:
        value = 0
        byte_data = struct.pack(">I", value)
        bits_data = bitarray(endian="little")
        bits_data.frombytes(byte_data)
    ...
    def test_DateTimeField_set_name(self) -> None:
        value = 0
        byte_data = struct.pack(">I", value)
        bits_data = bitarray(endian="little")
        bits_data.frombytes(byte_data)
    ...
    def test_DateTimeField_set_value(self) -> None:
        value1 = 0
        ...
        tst = ParseData(
            name="test",
            value=datetime.utcfromtimestamp(value1),
            string_format=DATE_FIELD_STRING_FORMAT,
        ...
        obj.value = datetime.utcfromtimestamp(value2)
        tst.value = datetime.utcfromtimestamp(value2)
